...
from A_utils.torch_utils import evaluate_chordfile, recomposition_c170
from D_BTCmodel.model import BTC_model
from D_CRNNmodel.model import CRNN_model
...
with open('A_utils/feature_config.yaml', 'r') as conf:
    feature_config = yaml.safe_load(conf)

with open(f'D_{args.model}model/model_config.yaml', 'r') as conf:
...
kf = np.load('A_utils/split_indices.npz')
test_files = kf['test']
...
for index in range(1, 6):
    model.load_state_dict(torch.load(f'D_{args.model}model/saved_models/model_{index}.pth'))

    std_data = np.load(f'A_utils/standardization/std_{index}.npz')
    ...
    for audio_file in tqdm(test_files):
        y, _ = librosa.load(f'B_audiofiles/{audio_file}')
        ...
        labdir = f'Z_temp/est_lab/idx_{index}'
        ...
        ref_lab = f'B_chordfiles/{lab_name}'
